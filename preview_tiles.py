...
class Preview_Tiles:
    def __init__(self):
        ...
        self.gold = pygame.image.load(con.PREVIEW_PIT)
    ...
    def background(self, surface):
        for i in range(con.ROW_COUNT):
            for j in range(con.COL_COUNT):
                surface.blit(self.visible_floor, (j * con.PREVIEW_TILE_SIZE, i * con.PREVIEW_TILE_SIZE))
                if self.obstacle[i][j] == 'p':
                    surface.blit(self.pit, (j * con.PREVIEW_TILE_SIZE, i * con.PREVIEW_TILE_SIZE))
                elif self.obstacle[i][j] == 'w':
                    surface.blit(self.wumpus, (j * con.PREVIEW_TILE_SIZE, i * con.PREVIEW_TILE_SIZE))
                elif self.obstacle[i][j] == 'g':
                    gold_back = pygame.image.load('./res/floor_gold.png')
                    surface.blit(gold_back, (j * con.PREVIEW_TILE_SIZE, i * con.PREVIEW_TILE_SIZE))
                    surface.blit(self.gold, (j * con.PREVIEW_TILE_SIZE, i * con.PREVIEW_TILE_SIZE))

        for i in range(con.ROW_COUNT):
            pygame.draw.line(surface, con.WHITE, (0, i * con.PREVIEW_TILE_SIZE), (self.width, i * con.PREVIEW_TILE_SIZE))
        pygame.draw.line(surface, con.WHITE, (0, self.height - 1), (self.width, self.height - 1))

        for i in range(con.COL_COUNT):
            pygame.draw.line(surface, con.WHITE, (i * con.PREVIEW_TILE_SIZE, 0), (i * con.PREVIEW_TILE_SIZE, self.height))
